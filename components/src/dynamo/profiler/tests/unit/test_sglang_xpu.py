...
class TestSetDeviceTypeXpu:
    ...
    def test_template_name_derived_from_metadata(self) -> None:
        ...
        assert any(
            rc.get("resourceClaimTemplateName")
            == "my-sglang-deployment-decode-gpu-template"
    ...
    def test_disagg_different_tp_sizes_get_separate_templates(self) -> None:
        ...
        prefill_count = prefill_tpl["spec"]["spec"]["devices"]["requests"][0][
            "exactly"
        ]["count"]
        decode_count = decode_tpl["spec"]["spec"]["devices"]["requests"][0]["exactly"][
            "count"
        ]
        assert (
            prefill_count == 2
        ), f"prefill template has wrong GPU count: {prefill_count}"
